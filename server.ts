...
async function checkAccountConnection(account: StoredAccount): Promise<{
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Extract Account Status & Expiry Date from Dashboard HTML
      let accountStatus: 'Active' | 'Expired' | 'Unknown' = 'Unknown';
      let expiryDate: string | null = null;

      $resp('*').each((_, el) => {
        const text = $resp(el).text().trim();
        if (text === 'Account Status' || text.toLowerCase().includes('account status')) {
          const parent = $resp(el).parent();
          const valText = parent.find('p, font, span').text().trim() || parent.siblings('p').text().trim() || parent.next().text().trim();
          if (valText) {
            const u = valText.toUpperCase();
            if (u.includes('ACTIVE')) accountStatus = 'Active';
            else if (u.includes('EXPIRED') || u.includes('INACTIVE')) accountStatus = 'Expired';
          }
        }
        if (text === 'Expiry Date' || text.toLowerCase().includes('expiry date')) {
          const parent = $resp(el).parent();
          const dateVal = parent.find('p, span').text().trim() || parent.siblings('p').text().trim() || parent.next().text().trim();
          if (dateVal && !dateVal.toLowerCase().includes('expiry date')) {
            expiryDate = dateVal;
          }
        }
      });
...
app.get('/api/export', (req, res) => {
  ...
  // Default CSV format
  let csvContent = 'Username,Connection Status,Account Status,Expiry Date,Last Checked,Response Time (ms),Error\n';
  for (const acc of accounts) {
    csvContent += `"${acc.username}","${acc.status}","${acc.accountStatus || 'Unknown'}","${acc.expiryDate || 'N/A'}","${
      acc.lastChecked || 'N/A'
